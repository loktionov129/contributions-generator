...
function handleCursorMoving(event) {
    const { target } = event;

    if (this._previousRect === target || target.tagName !== 'rect') {
        return;
    }

    this._previousRect = target;

    this._changeContributions(event);
}

function enableDrawing(event) {
    preventMissClick(event);
    this._contributions.addEventListener('mousemove', this._handleCursorMoving);
}
...
class AppView {
    constructor() {
        ...
        this._handleCursorMoving = handleCursorMoving.bind(this);
